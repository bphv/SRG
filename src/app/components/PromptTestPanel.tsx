  ...
  return (
    <div className="rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-6 shadow-[0_18px_34px_rgba(30,90,72,0.08)]">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.22em] text-[var(--lagoon-deep)]">Test prompt</p>
          <p className="text-sm text-[var(--sea-ink-soft)]">Exécutez votre prompt avec un provider et un modèle.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label className="block text-sm font-semibold text-[var(--sea-ink)]">Provider</label>
          <select
        ...
        <div>
          <label className="block text-sm font-semibold text-[var(--sea-ink)]">Modèle</label>
          <select
            value={selectedModel}
      ...
      <div className="mt-5 rounded-[1.75rem] border border-[var(--line)] bg-[var(--surface-strong)] p-4">
        <p className="text-sm font-semibold text-[var(--sea-ink)]">Variables d’exécution</p>
        <div className="mt-4 grid gap-4">
          {availableVariables.length === 0 ? (
            <p className="text-sm text-[var(--sea-ink-soft)]">Aucune variable définie pour le prompt sélectionné.</p>
          ) : (
            availableVariables.map((variable) => (
              <div key={variable.name} className="grid gap-2">
                <label className="text-sm text-[var(--sea-ink)]">{`{{${variable.name}}}`}</label>
                <input
      ...
      <button
        ...
        onClick={handleRun}
        disabled={!prompt}
        className="mt-5 rounded-3xl bg-[var(--lagoon-deep)] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[var(--palm)] disabled:cursor-not-allowed disabled:opacity-50"
      >
        Exécuter
      </button>

      {result ? (
        <div className="mt-4 rounded-[1.75rem] border border-[var(--line)] bg-[var(--surface-strong)] p-4 text-sm text-[var(--sea-ink)]">
          <p className="font-semibold text-[var(--sea-ink)]">Réponse</p>
