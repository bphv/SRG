...
function SkeletonBlock({ className }: { className: string }) {
  return <div className={`animate-pulse rounded-3xl bg-[var(--srg-surface-strong)] ${className}`} aria-hidden />
}
  ...
  return (
    <div className="space-y-6" role="status" aria-live="polite" aria-label={title ?? 'Chargement du workspace'}>
      <div className="rounded-[2rem] border border-[var(--srg-border)] bg-[var(--srg-surface)] p-6 shadow-[var(--srg-shadow-md)]">
        ...
        <p className="mt-5 text-sm text-[var(--srg-text-muted)]">{description ?? 'Préparation de la vue et des données du workspace.'}</p>
      </div>

      <div className={heroGridClasses[variant]}>
        {Array.from({ length: layout.heroCards }).map((_, index) => (
          <div key={`hero-${variant}-${index}`} className="rounded-[2rem] border border-[var(--srg-border)] bg-[var(--srg-surface)] p-5 shadow-[var(--srg-shadow-md)]">
      ...
      <div className={bodyGridClasses[variant]}>
        {Array.from({ length: layout.bodyColumns }).map((_, columnIndex) => (
          <div key={`panel-${variant}-${columnIndex}`} className="rounded-[2rem] border border-[var(--srg-border)] bg-[var(--srg-surface)] p-6 shadow-[var(--srg-shadow-md)]">
